...
def forward_propagation(x, theta):
    """
    Implement the linear forward propagation (compute J) presented in Figure
    1 (J(theta) = theta * x)
...
def gradient_check(x, theta, epsilon=1e-7):
    ...
    Returns:
    difference -- difference (2) between the approximated gradient and the backward
...
def gradient_check_n(parameters, gradients, X, Y, epsilon=1e-7):
    ...
    "W3", "b3":
    grad -- output of backward_propagation_n, contains gradients of the cost with respect
    to the parameters.
    x -- input datapoint, of shape (input size, 1)
    y -- true "label"
    epsilon -- tiny shift to the input to compute approximated gradient with formula(1)
    ...
    Returns:
    difference -- difference (2) between the approximated gradient and the
    ...
    for i in range(num_parameters):
        ...
        # "_" is used because the function you have to outputs two parameters but
        # we only care about the first one
        ### START CODE HERE ### (approx. 3 lines)
        thetaplus = np.copy(parameters_values)
        # Step 1
        thetaplus[i][0] = parameters_values[i] + epsilon
        # Step 2
        ...
        # Step 1
        thetaminus[i][0] = parameters_values[i] - epsilon
        # Step 2
